...
const LyricView: React.FC<LyricViewProps> = ({ player }) => {
  ...
  const openComments = useCallback(() => {
    navigation.push(ViewType.COMMENT, {
      releaseId: currentTrack?.freeflow?.releaseId ?? null,
      trackTitle: currentTrack?.title,
    });
  }, [currentTrack?.freeflow?.releaseId, currentTrack?.title, navigation]);
  ...
  const handleWheel = useCallback((event: React.WheelEvent<HTMLDivElement>) => {
    ...
    while (el && el !== event.currentTarget) {
      if (el.scrollHeight > el.clientHeight + 8) {
        const canScrollDown = el.scrollTop + el.clientHeight < el.scrollHeight - 8;
