...
export default function WorkshopTopBar({
  ...
  const walletLabel = controller.isConnected && controller.address
    ? `重启并切换 Profile (${controller.address.slice(0, 6)}...)`
    : '重启并选择 Profile';
  ...
  return (
    <header className={styles.topBar}>
      ...
      <div className={styles.topBarActions}>
        ...
        >
          {controller.authBusy ? '处理中...' : (controller.web25Session ? '退出并重启到 Guide' : '重启并前往 Guide 登录')}
        </button>
        <button
          className={styles.walletButton}
          onClick={() => {
            void controller.handleRestartToGuide();
          }}
        >
          {walletLabel}
        </button>
