...
系统安全主设计要集中在身份建立、访问控制、交易执行、接口保护以及资源访几个方面。本系统使用 SIWE 签名登录机制，后端会对 nonce、地址、链 ID、domain、uri 签发时间等进行校验并恢复出钱包地址和传入地址比对,nonce 具有过期时间和消费限制,以防范重放攻击.
...
在资源保护上,当前模式下,如果资源地址被直接获取，仍有可能绕过应用这边的访问流程。已经有研究表明，链上+链下的混和去中心化应用架构中,链上的确权信息仍需配合链下的加密、密钥管理或类似lit protocol等去中心化鉴权模式才能有效防止盗链\cite{Ali2017IoTIPFS,Qin2021BlockchainAccess}。现阶段的访问控制，只是原型系统的授权验证机制，后续需要结合音频加密、授权网关来完善资源保护.
