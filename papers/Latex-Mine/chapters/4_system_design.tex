...
FreeFlow采用桌面端、链下服务、智能合约与IPFS存储协同的整体架构。桌面端负责用户交互、钱包连接、播放控制与本地缓存；后端负责认证、上传代理、资源索引、购买记录与评论服务；智能合约负责作品发布、访问授权与收益分账；IPFS/Pinata负责音频、封面与metadata资源存储。系统总体架构如图\ref{fig:system_architecture}所示。

在职责划分上，作品发布结果、访问凭证、交易记录等关键状态由链上维护，评论、会话、搜索索引和上传记录等高频业务数据由链下维护。该分层方式能够在链上可信记录与链下处理效率之间形成较稳妥的工程折中，并降低链上应用在同步、成本和交互延迟方面的实现压力\cite{Wei2022BDMSSurvey,Six2022DAppPatterns,DArcher2021}。。
...
创作者发布流程如图\ref{fig:release_workflow}所示。该流程从发行草稿创建开始，依次完成素材上传、metadata生成、链上发布与结果回写。媒体资源由IPFS保存，链上记录metadata URI、访问配置与发行结果等关键状态。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/out/purchase_playback_sequence.png}
    \caption{用户购买与播放流程图}
...
系统采用客户端SQLite数据库与服务器PostgreSQL数据库的双数据库方案。前者服务于本地音乐管理、缓存与播放状态维护，后者服务于身份认证、资源索引、上传记录、购买记录与评论存储。双数据库设计能够分别满足桌面端本地能力与链下业务协同的需要。

\subsection{客户端SQLite数据库设计}

客户端SQLite数据库以统一歌曲记录为核心，其ER关系如图\ref{fig:client_sqlite_er}所示。\texttt{track}表承载本地音乐、外部来源音乐与FreeFlow链上音乐的统一表示；歌单与会话信息则围绕该表组织。链上音乐的扩展字段序列化保存在\texttt{freeflow\_metadata\_json}中，以复用本地播放、缓存与下载逻辑。
...
服务器PostgreSQL数据库的ER关系如图\ref{fig:server_postgresql_er}所示。后端以\texttt{creator\_releases}为资源索引中心，关联用户、钱包身份、存储对象、购买记录与评论数据。该设计支持资源检索、详情展示、阶段恢复和交易投影等业务需求。
...
在约束设计上，\texttt{creator\_releases}通过\texttt{chainId + musicAssetAddress + tokenId}保证链上资源索引唯一；\texttt{purchases}通过交易哈希与用户-资源组合约束防止重复确认；\texttt{comments}通过资源与用户索引支持评论列表和用户行为查询。
...
系统安全设计围绕身份建立、访问控制、交易执行、接口保护和资源访问边界展开。在身份认证方面，系统采用 SIWE 签名登录机制，后端对 nonce、地址、链 ID、domain、uri 和签发时间进行校验，以降低签名重放和身份伪造风险。

在访问控制方面，付费作品的访问资格以链上凭证与合约访问结果为依据，客户端仅负责发起查询与展示结果，不直接决定最终权限。购买流程中的价格、销售状态和凭证发放均由合约完成校验与执行，从而使关键访问规则能够在链上形成可验证、可审计的统一约束\cite{Maesa2019AuditableAC,Qin2021BlockchainAccess,Punia2024AccessReview}。

在接口保护方面，上传、发行、购买和评论等链下接口均要求会话认证，并通过服务端统一维护用户身份、上传记录和交易投影。当前系统的音频资源仍基于 CID 和网关访问，若资源地址被直接获取，仍可能绕过应用侧访问流程。已有研究也表明，在区块链与 IPFS 结合的数据共享场景中，链上访问授权通常仍需配合链下加密、密钥管理或受控解密流程，才能更完整地约束实际资源访问\cite{Ali2017IoTIPFS,Qin2021BlockchainAccess}。因此，现阶段的访问控制更适合作为原型系统中的授权验证机制，后续仍需结合音频加密、授权网关或短期签名 URL 等方案进一步完善资源保护能力。
