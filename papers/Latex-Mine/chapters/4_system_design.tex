...
创作者发布流程如图\ref{fig:release_workflow}所示。该流程从发行草稿创建开始，依次完成素材上传、 metadata 生成、链上发布与结果回写。媒体资源由 IPFS 保存，链上记录 metadata URI、访问配置与发行结果等关键状态。
...
客户端 SQLite 数据库以统一歌曲记录为核心，其 ER 关系如图\ref{fig:client_sqlite_er}所示。\texttt{track}表承载本地音乐、外部来源音乐与 FreeFlow 链上音乐的统一表示；歌单与会话信息则围绕该表组织。链上音乐的扩展字段序列化保存在\texttt{freeflow\_metadata\_json}中，以复用本地播放、缓存与下载逻辑。
...
系统的安全设计，主要围绕身份建立、访问控制、交易执行、接口保护以及资源访问边界这几个方面来展开。在身份认证方面，系统运用了 SIWE 签名登录机制，后端会对 nonce、地址、链 ID、domain、uri 还有签发时间这些字段进行校验，借此来降低签名重放和身份伪造所带来的风险。

在访问控制方面，付费作品的访问资格是把链上凭证以及合约访问结果当作依据的，客户端只去发起查询并且展示结果，而不直接决定最终的权限。购买流程里涉及的价格、销售状态还有凭证发放，这些全部都由合约来完成校验与执行，这样关键访问规则就能够在链上形成一种可验证而且可审计的统一约束\cite{Maesa2019AuditableAC,Qin2021BlockchainAccess,Punia2024AccessReview}。

在接口保护方面，像上传、发行、购买以及评论这些链下接口，全都要求进行会话认证，并且借助服务端来统一维护用户身份、上传记录还有交易投影。目前系统里的音频资源，依然是基于 CID 还有网关来进行访问的，如果资源地址被别人直接拿到，还是有可能绕过应用这边的访问流程。已经有研究也表明了，在区块链和 IPFS 相结合的数据共享场景当中，链上的访问授权通常还是需要配合链下的加密、密钥管理或者是受控解密流程，才可以更加完整地约束实际的资源访问\cite{Ali2017IoTIPFS,Qin2021BlockchainAccess}。所以，现阶段的访问控制，更加适宜当作原型系统里的授权验证机制，后续还是需要结合音频加密、授权网关或者短期签名URL等方案，来进一步完善资源保护方面的能力。
