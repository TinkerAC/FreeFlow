...
在职责划分上，作品发布结果、访问凭证、交易记录等关键状态由链上维护，评论、会话、搜索索引和上传记录等高频业务数据由链下维护。该分层方式能够兼顾状态可信性、检索效率与系统响应能力\cite{Eren2025,Friedl2024}。

\begin{figure}[H]
    \centering
    \figmax{figures/out/system_architecture.png}{}
...
\begin{figure}[H]
    \centering
    \figmax{figures/out/release_workflow.png}
...
\begin{figure}[H]
    \centering
    \figmax{figures/out/purchase_playback_sequence.png}
    \caption{用户购买与播放流程图}
...
在访问控制方面，付费作品的访问资格以链上凭证与合约访问结果为依据，客户端仅负责发起查询与展示结果，不直接决定最终权限。购买流程中的价格、销售状态和凭证发放均由合约完成校验与执行，从而保证关键业务规则由链上统一约束。

在接口保护方面，上传、发行、购买和评论等链下接口均要求会话认证，并通过服务端统一维护用户身份、上传记录和交易投影。需要说明的是，当前系统的音频资源仍基于 CID 和网关访问，若资源地址被直接获取，仍可能绕过应用侧访问流程。因此，现阶段的访问控制更适合作为原型系统中的授权验证机制，后续仍需结合音频加密、授权网关或短期签名 URL 等方案进一步完善资源保护能力。
