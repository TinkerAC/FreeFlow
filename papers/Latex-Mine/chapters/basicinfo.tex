...
\newcommand{\abstractCN}{
    ...
    为提升链上音乐资源的可检索性，本文进一步设计了链上音乐资源检索与排序模块。该模块综合考虑标题、艺术家、专辑、流派、描述等文本字段，以及合约地址、Token ID、交易哈希和IPFS CID等链上身份字段，通过查询归一化、候选召回、特征评分、模糊相似度、新鲜度和热度计算完成排序，并返回可解释的排序原因。系统测试从智能合约自动化测试和检索排序离线评估两个方面进行验证。结果表明，合约自动化测试4项全部通过；在包含56条资源和62个查询的评估集上，完整排序算法的MRR达到0.9758，nDCG@10达到0.9646，在500条候选规模下平均响应时间为36.09 ms，系统能够完成从创作者发布到用户购买播放的链上音乐业务闭环。
}
