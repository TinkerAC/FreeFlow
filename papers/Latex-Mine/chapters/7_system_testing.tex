...
FreeFlow同时涉及桌面端、后端、智能合约与外部IPFS服务。由于部分功能依赖桌面端本地代理、外部存储服务与用户交互流程，本文将验证工作划分为两类可稳定复现的任务：一类是基于Hardhat本地链的智能合约自动化测试，用于验证发布、购买和访问控制等链上核心逻辑；另一类是基于离线评估集的检索排序实验，用于验证第6章提出的资源发现策略。对于登录、素材上传、详情展示、播放和评论等桌面端与后端协同功能，本文在第五章以业务链路形式说明其实现与联调结果，但不将其表述为已经完成大规模自动化端到端实验。
...
该结果表明，\texttt{PlatformHub}与\texttt{MusicAccess1155}之间的协作逻辑能够正确支撑作品发布、购买和访问校验；同时，创作者侧收益分配能够按预设流程导入\texttt{RoyaltySplitter}。这为桌面端调用链上接口提供了可靠前提，但并不等同于已经完成生产环境安全审计或大规模链上压力测试。
...
需要指出的是，该评估集主要服务于当前系统在冷启动场景下的可复现实验，而非模拟真实线上平台的全部流量特征。为便于调试和解释，部分链上身份字段采用结构化测试标识，因此实验结果更适合说明排序机制在当前实现中的工程有效性，而不应直接外推为生产环境中的最终搜索质量。
...
同时，本章也有明确边界。桌面端的登录、上传、详情展示、播放、评论和下载等功能已经完成实现与联调，但尚未形成大规模自动化端到端测试，因此本文不将其表述为“全部经过严格实验验证”的结论。检索实验所使用的评估集是结构化模拟数据，适合验证算法链路和特征设计是否工作正常，但不足以替代真实用户搜索日志和线上A/B测试。因而，本文更适合将结论表述为：FreeFlow完成了面向音乐场景的混合架构实现验证，而不是已经达到生产环境中的完整平台成熟度。


\section{本章小结}

本章围绕当前实现中可复现的两类核心任务完成了验证。结果表明，智能合约核心逻辑能够稳定支撑作品发布、购买与权限控制；链上资源排序模块在引入身份字段后，MRR提升到0.9758、nDCG@10提升到0.9646，并将零结果率降至0；在500条候选规模下，完整排序算法仍可保持36.09 ms的平均响应时间。结合第五章的业务链路实现，可以认为FreeFlow已经完成了“链上发行与访问控制 + 链下索引与基础互动”的系统级实现验证。
