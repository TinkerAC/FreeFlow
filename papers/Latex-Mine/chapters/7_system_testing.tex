...
智能合约测试基于Hardhat测试框架完成，围绕作品发布、购买授权、访问控制和销售配置修改等核心场景设计自动化用例。测试总计4项，全部通过，总耗时为758 ms。测试结果如表\ref{tab:contract_tests}所示。
...
本章基于统一实验流程完成了FreeFlow系统的实验验证。结果表明，智能合约核心逻辑已经能够稳定支撑作品发布、购买与权限控制；链上资源排序模块在引入身份字段后，MRR提升到0.9758、nDCG@10提升到0.9646，并将零结果率降至0；在500条候选规模下，完整排序算法仍可保持36.09 ms的平均响应时间。综合来看，FreeFlow已经具备较完整的链上音乐发布、访问和检索能力。
