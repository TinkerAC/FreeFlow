...
export interface KnobProps { value: number; min?: number; max: number; onChange: (v:number)=>void; showTimes?: boolean; className?: string; }
...
export const Knob: React.FC<KnobProps> = ({ value, min = 0, max, onChange, showTimes = true, className }) => {
  ...
  const size = 72; const r = 28; const cx = size/2; const cy = size/2; const C = 2 * Math.PI * r;
  const dash = useMemo(() => ({ array: C, offset: (1 - pct) * C }), [C, pct]);

  const toPctFromClient = (clientX: number, clientY: number) => {
    const el = ref.current; if (!el) return pct;
  ...
  return (
    <div className={clsx(styles.root, className)} tabIndex={0} onKeyDown={onKeyDown} {...aria}>
      <svg ref={ref} className={styles.svg} viewBox={`0 0 ${size} ${size}`} onPointerDown={onPointerDown}>
        ...
        <circle className={styles.indicator} cx={cx} cy={cy} r={r} strokeDasharray={dash.array} strokeDashoffset={dash.offset} />
