...
import { getMusicLink } from '@main/services/hifiniMusicService';
...
// 启动代理服务器的函数
export async function startProxyServer(db: Database, store: any): Promise<void> {
    // 从配置或环境变量中获取端口号
    let port =  4399;
    let cacheTime = store.get('cacheTime') || 86400; // 缓存时间默认为 1 天
    console.log('代理服务器数据库已连接');

    // 创建一个 GET 接口，接受客户端请求并转发到目标服务器
    app.get('/proxy', async (req, res) => {
        const dataHref = req.query.dataHref as string; // 从请求中获取 dataHref 参数
                ...
                transform(chunk, encoding, callback) {
                    if (isFirstChunk) {
                        ...
                        if (chunkString.trim() === '-1') {
                            ...
                            // 将响应头发送给客户端
                            res.writeHead(proxyRequest.response?.statusCode || 200, proxyRequest.response?.headers);
                        }
                    }
                    // 将数据写入响应
                    res.write(chunk);
