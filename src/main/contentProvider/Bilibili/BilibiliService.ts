...
export class BilibiliService {
    ...
    constructor() {
        ...
        //对主站发起一次get
        this.http.get("https://www.bilibili.com")
    ...
    /**
     * 解析 Bilibili 搜索响应，提取所有视频的 bvid
     * @param {any} responseData - API 返回的 JSON 对象（即 resp.data）
     * @returns {string[]} 包含所有 bvid 的字符串数组
     */
    public static extractBvids(responseData: any): string[] {
        // 1. 提取安全的 result 数组引用（使用多层可选链防止 data 为空）
        ...
        // 3. 找到 result_type 为 'video' 的模块（直接使用提取好的 resultList）
        const videoModule = resultList.find(
            (item: any) => item.result_type === 'video'
        );

        // 4. 提取 bvid
        if (videoModule && Array.isArray(videoModule.data)) {
            return videoModule.data.map((video: any) => video.bvid);
        }
