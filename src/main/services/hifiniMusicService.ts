...
async function search(keyword: string) {
  const extractLiElements = (html: any) => {
...
// Base32 编码函数
function base32Encode(str: string) {
...
function isRedirectedUrlValid(redirected_url: string) {
...
// 获取音乐链接的函数
export async function getMusicLink(dataHref: string, db: Database
  , store: any) {
  try {
    // 从数据库中获取相关记录
    const row: any
      = await dbGet(db, 'SELECT un_redirected_url, cached_at FROM hifini_info WHERE data_href = ?', [dataHref]);
...
// 获取音乐信息的函数
export async function getMusicInfo(dataHref: string, db: Database, store: any) {

  try {
    const row: any = await dbGet(db, 'SELECT title, artist, cover_src, cached_at FROM hifini_info WHERE data_href = ?', [dataHref]);
    if (row) {
...
// 抓取并保存音乐信息的辅助函数
async function fetchAndSaveMusicInfo(dataHref: string
  , db: Database
  , store: any,
) {
...
// 过滤、排序结果并获取音乐信息
export async function getSearchResults(keyword: string, db: Database, store: any) {
  try {
